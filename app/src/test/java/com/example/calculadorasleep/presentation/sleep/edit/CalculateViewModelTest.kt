...
class CalculateViewModelTest {
    ...
    fun setup() {
        ...
        viewModel = CalculateViewModel(calculateSleepTimesUseCase, resolveSleepSessionMillisUseCase, saveSleepUseCase, alarmRepository, alarmScheduler, themeState)
    }

    @Test
    fun `al calcular se obtienen las opciones y no se selecciona ninguna por defecto`() = runTest {
        val options = listOf(
            SleepTimeOption(LocalTime.of(23, 0), 5, 7.5, true)
        )
